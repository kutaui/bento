    ...
    return <a href={href} target="_blank" className="group">
        <Card className={cn("w-64 bg-zinc-900 transition rounded-3xl h-36 overflow-hidden hover:scale-[1.02] hover:border-[3px] hover:cursor-pointer bg-cover",background)} >
            <CardHeader className="relative pt-10">
                <CardTitle className="text-white translate-y-6 group-hover:translate-y-0 transition text-xl font-semibold">
                    {image && <Image src={image} alt="asd" width={25} height={20} className="pb-2"/>}
                    {title}
                </CardTitle>
                <CardDescription className="text-white font-medium translate-y-6 group-hover:translate-y-0 z-10 opacity-0 group-hover:opacity-100 transition">{description}</CardDescription>
            </CardHeader>
        </Card>
    </a>;
};